...
├── plugin/
│   ├── mod.rs          # 插件系统公共接口（PluginManager 等）
│   ├── manager.rs      # 插件管理器：加载 / 卸载 / Hook 总线 / system_prompt 收集
│   ├── loader.rs       # plugin.toml 解析 + 目录扫描
│   ├── tool_loader.rs  # 插件 tools/ tool.json 动态注册
│   ├── skill_loader.rs # 插件 skills/ 注入
│   ├── metadata.rs     # 插件元数据类型定义
│   ├── cache.rs        # 工具定义缓存
│   └── scope.rs        # 插件作用域管理
└── tools/
...
插件是对 Agent 功能最自然的扩展单元，将目录放入 `.agent/plugins/` 并重启 Agent 即生效，无需修改任何 Rust 代码。
...
| 模式 | 行为 | 典型用途 |
|------|------|----------|
| `fire_and_forget` | 异步触发，不阻塞 Agent | 写日志、发通知 |
| `blocking` | 同步执行，Agent 等待后继续 | 启动前探活检查 |
| `intercepting` | stdout 可修改 Agent 决策 | 路由覆盖、工具拦截 |
...
| 事件 | 触发时机 | 支持 intercepting |
|------|----------|-----------|
| `agent.start` | Agent 初始化完成后 | ❌ |
| `tool.before` | 工具执行前 | ✅ |
| `tool.after` | 工具执行后 | ❌ |
...
结合 `.agent/system_prompt.md` 的 `# OVERRIDE` 机制可构建专属 Agent：

```
.agent/system_prompt.md   # OVERRIDE → 替换默认角色定位（如"你是一个新闻抓取机器人"）
.agent/plugins/news/
  ├── system_prompt.md    # 追加领域规则（抓取频率、来源过滤等）
  ├── tools/fetch_rss.json
  └── hooks/              # 定时触发、异常上报等
```
